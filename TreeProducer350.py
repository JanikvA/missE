...
#        fill(self.tree, 'absjet2res', abs(jet2res))
	...
	pvis=jets[0].p4()+jets[1].p4()
	m2=(365.-125./mvis*pvis.E())**2-(125./mvis*pvis.P())**2
	mmiss_r=-99
	if(m2>0):
		mmiss_r=math.sqrt(m2)

        fill( self.tree, 'mmiss_rescaled', mmiss_r )
        fill( self.tree, 'acop', higgs.acoplanarity() )
        fill( self.tree, 'mmiss_rescaled2', m2 )
